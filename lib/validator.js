    ...
    if (!self.types[typeName]) {
      ...
      _.each(bases, function (base) {
        ...
        resolved = resolved.concat(_.map(self.resolveType(xpath, base), function (res) {
          ...
          // Make sure not overriding some other type by accident
          var r = deepExtend({}, res, other);

          // If it is a restriction, limit children to those from other
          if (self.types[typeName].restriction) {
            r.children = _.pick(r.children, _.keys(other.children));
          }

          // Attributes can be restricted only in content
    ...
    for (var i = 0; i < type.length; i++) {
      ...
      try {
        ...
        if (type[i].anyChildren) {
          ...
          // Children namespace might be different than current namespace, so remove namespace both
          ...
          _.each(value, function (child, name) {
            ...
            //Remove namespace from current key name as well
...
function validator(xpath, currentValue, newValue, stack) {
  ...
  //Overriding given xpath with namespaced xpath
  xpath = '/' + path.join('/');
